...
public class EMAIndicatorBuilder implements IndicatorBuilder {
    ...
    public Indicator build() {
        return indicatorCache.ema(sourceIndicatorBuilder.build(), integerIterator.value());
    }

    @Override
    public Indicator buildDefault() {
        return indicatorCache.ema(sourceIndicatorBuilder.buildDefault(), integerIterator.defaultValue());
    }
